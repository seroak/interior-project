  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        setData(record);
        // Initialize selectedStyle with the first key if available
        if (record.generated_images && typeof record.generated_images === "object") {
          const keys = Object.keys(record.generated_images);
          if (keys.length > 0) {
      ...
      <main className="flex-1 container mx-auto px-4 py-8 flex flex-col items-center justify-center">
        <div className="w-full max-w-[1000px] flex flex-col gap-8">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {Object.keys(generatedImageUrl).map((style) => (
              <button
                key={style}
